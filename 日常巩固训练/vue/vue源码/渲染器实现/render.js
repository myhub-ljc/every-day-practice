const h = (tag, props, children) => {
  ...
  const patch = (n1, n2) => {
    //首先比较类型是否相同
    if (n1.tag !== n2.tag) {
      //如果元素类型不同,需要拿到该元素的父元素从而进行一个移除
      const n1ElParent = n1.el.parentElement;
      n1ElParent.removeChild(n1.el);
      //最后再将其余部分继续挂载到新的虚拟DOM上面
      mount(n2, n1ElParent);
    } else {
      //当只有元素属性不同时的处理方式
      ...
      // 2.1.获取所有的newProps添加到el(也就是覆盖旧的props)
      ...
      if (typeof newChidlren === "string") { // 情况一: newChildren本身是一个string
        ...
        if (typeof oldChildren === "string") {
          ...
          // 2.newChildren.length > oldChildren.length进行增加的操作
          ...
          // 3.newChildren.length < oldChildren.length进行删除的操作
          if (newChidlren.length < oldChildren.length) {
